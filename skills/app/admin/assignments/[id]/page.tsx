...
import { ArrowLeft, FileText, CheckCircle, XCircle, Save, Loader2, User, Clock, Award, Trash2 } from "lucide-react"
...
export default function AdminAssignmentDetailPage() {
  ...
  useEffect(() => {
    if (params.id) {
      fetch(`/api/admin/assignments/${params.id}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.success) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/assignments/${params.id}`, { method: "DELETE" })
      const data = await res.json()
      if (data.success) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border p-6">
        ...
        <p className="text-gray-500 mb-1"><span className="font-medium">Course:</span> {assignment.course?.title}</p>
        <p className="text-gray-500 mb-4"><span className="font-medium">Instructor:</span> {assignment.course?.instructor?.fullName || "N/A"}</p>
        ...
        {assignment.instructions && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-gray-600 whitespace-pre-wrap text-sm">{assignment.instructions}</p>
          </div>
        )}
      </div>
