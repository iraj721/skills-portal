  ...
  try {
    ...
    if (!courseId) {
      return NextResponse.json(
        {
          success: false,
          error: { code: "VALIDATION_ERROR", message: "Course ID required" },
        },
        { status: 400 },
      );
    }
    ...
    if (!course && payload.role !== "admin") {
      return NextResponse.json(
        {
          success: false,
          error: { code: "FORBIDDEN", message: "Not your course" },
        },
        { status: 403 },
      );
    }
    ...
    studentMap.forEach((data) => {
      ...
      data.attendancePercentage =
        total > 0 ? Math.round((data.totalPresent / total) * 100) : 0;
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: { code: "SERVER_ERROR", message: "Failed to mark attendance" },
      },
      { status: 500 },
    );
  }
